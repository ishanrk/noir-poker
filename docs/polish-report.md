...
Review the published diff and keep the original checkout intact. Follow release.md before any coordinated backend release, including compatible frontend and server identities, both additive migrations, backups, validation, and rollback compatibility. Do not revive an interrupted encrypted hand by rolling back the schema. The owner-authorized frontend was pushed to `oracle-deploy` and checked on the public domain. No server binary, database, infrastructure, package, or wallet state was changed.
